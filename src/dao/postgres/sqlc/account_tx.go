...
func (store *SqlStore) DeleteAccountWithTx(c context.Context, accountID int64) error {
	return store.execTx(c, func(queries *Queries) error {
		var err error
		err = tool.DoThat(err, func() error { return queries.DeleteFriendRelationsByAccountID(c, accountID) })
		return tool.DoThat(err, func() error { return queries.DeleteAccount(c, accountID) })
	})
}
