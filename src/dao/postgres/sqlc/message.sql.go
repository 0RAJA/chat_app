...
returning id, notify_type, msg_type, msg_content, msg_expand, file_id, account_id, rly_msg_id, relation_id, create_at, is_revoke, is_top, is_pin, pin_time, read_ids, msg_content_tsv
`
...
func (q *Queries) GetPinMsgsByRelationID(ctx context.Context, arg *GetPinMsgsByRelationIDParams) ([]*GetPinMsgsByRelationIDRow, error) {
	rows, err := q.db.Query(ctx, getPinMsgsByRelationID, arg.RlyMsgID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
